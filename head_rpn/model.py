...
def get_regularized_model(config):
    ...
    rpn_class = Conv2D(config['anchor_count'], (1, 1), activation='sigmoid', kernel_regularizer='l2', name='RPN_classifier')(output)
    rpn_regr = Conv2D(config['anchor_count'] * 4, (1, 1), activation='linear', kernel_regularizer='l2', name='RPN_regressor')(output)
    return Model(inputs=base_model.input, outputs=[rpn_class, rpn_regr])
